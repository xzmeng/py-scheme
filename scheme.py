...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if value is not None:
            return value
        raise SchemeError("Unknown identifier: '{0}' (lookup in {1})"
                          .format(symbol, self.__repr__()))
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('formal parameter and arguments count mismatch!')
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            if self.use_env:
                result = self.fn(*python_args, env)
            else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            r = eval_all(clause.second, env)
            return r if r is not None else test
        expressions = expressions.second
...
def make_let_frame(bindings, env):
    ...
    check_formals(formals)
    return env.make_child_frame(formals, vals)  # END PROBLEM 15
